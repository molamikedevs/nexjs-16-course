...
export async function getTags(
  ...
  try {
    ...
    const tags = await Tag.find(filterQuery).sort(sortOption).skip(skip).limit(limit);
...
export async function getTagQuestions(
...
): Promise<ActionResponse<{ tag: TagParams; questions: QuestionParams[]; isNext: boolean }>> {
  // 1. Validate input parameters and check if they are correct
  const validationResult = await action({
    params,
  ...
  try {
    ...
    return {
      success: true,
      data: {
        tag: JSON.parse(JSON.stringify(tag, null, 2)),
        questions: JSON.parse(JSON.stringify(questions, null, 2)),
        isNext,
      },
    };
  } catch (error) {
...
export async function getTopTags(): Promise<ActionResponse<TagParams[]>> {
  try {
    ...
    return { success: true, data: JSON.parse(JSON.stringify(tags, null, 2)) };
  } catch (error) {
    return handleError(error) as ErrorResponse;
  }
}
